...
void AutoMaintenanceOnLevelupAction::LearnQuestSpells(std::ostringstream* out)
{
    ...
    for (ObjectMgr::QuestMap::const_iterator i = questTemplates.begin(); i != questTemplates.end(); ++i)
    {
        ...
        // skip quests that are repeatable, too low level, or above bots' current level
...
void AutoMaintenanceOnLevelupAction::LearnSpell(uint32 spellId, std::ostringstream* out)
{
    ...
    // NOTE: Originally, AC casts spells instead of learning them, but we sacrifice
    // safe spell cast checks here in favor of performance and speed
